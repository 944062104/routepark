    ...
    <section className="py-16 px-4 bg-slate-50 max-w-7xl mx-auto w-full border-b border-slate-200/60">
      ...
      <div className="max-w-xl mx-auto mb-8 relative">
        ...
        <input
          ...
          className="w-full text-sm text-slate-800 placeholder-slate-400 bg-white border border-slate-200/80 rounded-xl pl-10 pr-4 py-3 outline-none focus:border-[#0B3D91] focus:ring-1 focus:ring-[#0B3D91] transition-all"
        />
        {searchQuery && (
          <button
        ...
        {filteredFAQ.length > 0 ? (
          filteredFAQ.map((item) => {
            ...
            return (
              <div 
                key={item.id}
                className="bg-white rounded-xl border border-slate-200/60 overflow-hidden hover:border-[#0B3D91]/25 transition-all shadow-sm border-l-4 border-l-[#0B3D91]"
              >
                {/* Header Acc Trigger */}
                <button
                  type="button"
                ...
                >
                  <span className="leading-snug pr-2 text-slate-800 hover:text-[#0B3D91]">
                    {item.question}
                  </span>
                  <span className="flex-none text-slate-400 mt-0.5">
                    {isOpen ? (
                      <ChevronUp className="h-5 w-5 text-[#0B3D91]" />
                    ) : (
      ...
      <div className="mt-8 text-center text-xs text-slate-400 flex items-center justify-center space-x-1">
        ...
        <span>以上解答遵循中华人民共和国《执业医师法》、《医疗机构管理条例》及民事居间担保条例。</span>
      </div>
    </section>
  );
}
